...
var (
	handshakerLogger = flogging.MustGetLogger("tendermint.handshaker")
)
...
func NewHandshaker(stateStore sm.Store, state sm.State, store sm.BlockStore, genDoc *tmtypes.GenesisDoc) *Handshaker {
	return &Handshaker{
		stateStore:   stateStore,
		initialState: state,
		store:        store,
		genDoc:       genDoc,
		eventBus:     tmtypes.NopEventBus{},
		nBlocks:      0,
		LatestState:  state,
	}
}
...
func (h *Handshaker) SetEventBus(eventBus tmtypes.BlockEventPublisher) {
	h.eventBus = eventBus
}

func (h *Handshaker) UpdateState(blockHeight int64, appHash []byte, responseInfo *abcitypes.ResponseInfo) {
	h.LatestState.LastBlockHeight = blockHeight
	h.LatestState.AppHash = appHash
}

func (h *Handshaker) DoHandshake(proxyApp proxy.AppConns) error {
	...
	handshakerLogger.Info("ABCI Handshake App Info")
	handshakerLogger.Info("InfoSync height", blockHeight)
	handshakerLogger.Info("InfoSync hash", appHash)
	handshakerLogger.Info("InfoSync software-version", res.Version)
	handshakerLogger.Info("InfoSync protocol-version", res.AppVersion)
	...
	finalAppHash, err := h.ReplayBlocks(h.initialState, appHash, blockHeight, proxyApp)
	if err != nil {
		return fmt.Errorf("error on replay: %v", err)
	}
	handshakerLogger.Info("finalAppHash", finalAppHash)

	h.UpdateState(blockHeight, finalAppHash, res)
	handshakerLogger.Info("Completed ABCI Handshake - Tendermint and App are synced",
...
func (h *Handshaker) ReplayBlocks(
	...
	if appBlockHeight == 0 {
		...
		if stateBlockHeight == 0 { // we only update state when we are in initial state
			...
			handshakerLogger.Infof("res.Validators %+v", res.Validators)
			...
			handshakerLogger.Infof("res.ConsensusParams %+v", res.ConsensusParams)
	...
	if storeBlockHeight == stateBlockHeight {
		...
		case appBlockHeight == stateBlockHeight:
			...
			handshakerLogger.Info("Replay last block using real app")
		...
		case appBlockHeight == storeBlockHeight:
			...
			handshakerLogger.Info("Replay last block using mock app")
...
func (h *Handshaker) replayBlocks(
	...
	for i := firstBlock; i <= finalBlock; i++ {
		handshakerLogger.Info("Applying block", "height", i)
		...
		appHash, err = sm.ExecCommitBlock(proxyApp.Consensus(), block, nil, h.stateStore, h.genDoc.InitialHeight)
		if err != nil {
			return nil, err
		}
...
func (h *Handshaker) replayBlock(state sm.State, height int64, proxyApp proxy.AppConnConsensus) (sm.State, error) {
	...
	blockExec := sm.NewBlockExecutor(h.stateStore, nil, proxyApp,
